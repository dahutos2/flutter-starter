...
```json
{
	...
	"Statement": [
		...
		},
		{
			...
			"Action": "iam:PassRole",
			"Resource": [
				"arn:aws:iam::765231401377:role/SageMakerOgiriTrainingJobRole"
			]
		},
		{
			"Effect": "Allow",
		...
		},
		{
			...
			"Resource": [
				"arn:aws:lambda:ap-northeast-1:765231401377:function:StartOgiriTrainingJob",
				"arn:aws:lambda:ap-northeast-1:765231401377:function:EndOgiriTrainingJob",
				"arn:aws:lambda:ap-northeast-1:765231401377:function:OgiriImageProcessing",
				"arn:aws:lambda:ap-northeast-1:765231401377:function:OgiriGetResults"
			]
		}
	]
}
...
ローカル環境で`dummy.json`ファイルを作成します。例えば以下の内容にします。
...
#### 7.6. トレーニングスクリプトを S3 にアップロード

1. `ogiri-training-data-bucket/empty`に「オブジェクトをアップロード」をクリックし、`dummy.json`をアップロードします。
    ...
    try:
        ...
        response = sagemaker.create_training_job(
            ...
            HyperParameters={
                ...
                'sagemaker_submit_directory': 's3://ogiri-training-data-bucket/',
            },
            ...
            InputDataConfig=[
                ...
                'DataSource': {
                    'S3DataSource': {
                    'S3DataType': 'S3Prefix',
                    'S3Uri': 's3://ogiri-training-data-bucket/empty/',
